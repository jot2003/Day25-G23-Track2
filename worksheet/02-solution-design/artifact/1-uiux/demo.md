...
## 5. Mermaid UI Flow — Luồng tương tác người dùng

```mermaid
flowchart TD
    Start([Người dùng mở SkyBot])

    subgraph normal [Trạng thái bình thường]
        NormalChat[Chat với SkyBot\nNút 👤 Nhân viên nhỏ, góc trên phải]
        PolicyMsg[SkyBot trả lời chính sách\n+ disclaimer footer nhỏ]
    end

    subgraph detection [Phát hiện bức xúc]
        Signal{Classifier + Prompt rule\nphát hiện ≥2 tín hiệu\nbức xúc?}
        Banner[Hiển thị Banner cam ⚡\n"Có vẻ bạn cần hỗ trợ khẩn cấp"\n+ Nút CTA lớn đỏ/cam]
        BotStop[SkyBot DỪNG giải thích chính sách\nChỉ xin lỗi + đề nghị chuyển]
    end

    subgraph escalation [Kết nối nhân viên]
        UserClick{Người dùng\nbấm nút?}
        WaitScreen[Banner xanh ✅\nMã yêu cầu #SK-xxxx\nThời gian chờ ước tính]
        AgentChat[Chat trực tiếp với nhân viên\nNhân viên có toàn bộ lịch sử]
        SelfExit[Người dùng tự bấm\nnút 👤 Nhân viên bất cứ lúc nào]
    end

    subgraph fallback [Fallback]
        QueueFull[Queue đầy?\nChuyển Hotline 1900-xxxx]
    end

    Start --> NormalChat
    NormalChat --> PolicyMsg
    PolicyMsg --> Signal
    NormalChat --> SelfExit

    Signal -->|Không| NormalChat
    Signal -->|Có| Banner
    Banner --> BotStop
    BotStop --> UserClick

    UserClick -->|Có| WaitScreen
    UserClick -->|Không — tiếp tục chat| NormalChat
    SelfExit --> WaitScreen

    WaitScreen --> AgentChat
    WaitScreen --> QueueFull
```
